...
"
" This plugin displays a list of all lines with word 'sub' in current
...
function! s:index()
    ...
    while (l:lnc <= b)
        ...
        let l:matched = matchlist(l:line,'\s*sub\s*[*&]*\([a-zA-Z0-9_]*\)\s*(')
        if( ! empty(l:matched))
            if(!empty(l:matched[1]))
                let l:line =  ' '.l:matched[1]
            else 
                let l:funcpos = stridx(tolower(l:line),'sub')
                if(l:funcpos>8) 
